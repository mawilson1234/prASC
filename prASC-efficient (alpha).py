...
if not args.nosentences:
	...
	if not args.resentences:
		...
		# If we're not combining results, look in the base results.csv
		if args.nocombine:
			# Make sure the results.csv exists to read from
			if os.path.isfile(csv_loc):
				try:
					#Read in the CSV and make it into a dataframe, then remove it so we can write it out again with the new sentences added later
					s_existing_results = pandas.read_csv(csv_loc, encoding = file_encoding)
					os.remove(csv_loc)
				except:
					print("Unable to read in existing results.csv. All sentences will be reprocessed.")
			else:
				print("No existing results.csv file found. All sentences will be reprocessed.")
		# If we are combining results, look in the results_combined.csv
		else:
			if os.path.isfile(output_dir / 'results_combined.csv'):
				s_existing_results = pandas.read_csv(output_dir / 'results_combined.csv', encoding = file_encoding, low_memory = False)
			else:
				print("No existing results_combined.csv found. All sentences will be reprocessed.")

		# If we found existing results
		if not s_existing_results.empty:
			s_existing_results = pandas.DataFrame(s_existing_results)
			s_existing_results = s_existing_results.loc[:, 'experiment_name':'value']
	...
	if args.nocombine and os.path.isfile(csv_loc) and not args.overwrite:
		print("Error: results.csv already exists in " + str(output_dir) + ". Use '--overwrite' ('-o') to overwrite existing results.csv files. Not processing sentences with SideEye.")
	elif not args.nocombine and os.path.isfile(output_dir / 'results_combined.csv') and not args.overwrite:
		print("Error: results_combined.csv already exists in " + str(output_dir) + ". Use '--overwrite' ('-o') to overwrite existing results_combined.csv files. Not processing sentences with SideEye.")
	else:
...
if not args.noquestions:
	...
	if not args.requestions:
		...
		# If we're not combining results, look in the base question files
		if args.nocombine:
			# If both questions files already exist, pull from the one with the least number of subjects
			if os.path.isfile(subj_quest_file_name) and os.path.isfile(summary_file_name):
				try:
					...
					print("Error: unable to read in and process existing question files. All questions will be reprocessed.")
			# If one question file is missing, we should reprocess all the questions (since we're missing all the info in the missing file)
			else:
				# Display the right error message
				if not os.path.isfile(subj_quest_file_name) and os.path.isfile(summary_file_name):
					print("No existing subject_question_info.txt file found. All questions will be reprocessed.")
				elif os.path.isfile(subj_quest_file_name) and not os.path.isfile(summary_file_name):
					print("No existing question_summary.txt file found. All questions will be reprocessed.")
				# Both are missing
				else:
					print("No existing questions files found. All questions will be reprocessed.")
		# If we are combining results, look in the combined_results.csv
		else:
			if os.path.isfile(output_dir / 'results_combined.csv'):
				print("Warning: recreating question files from results_combined.csv. Information about practice question responses will be lost for existing question data, and some very slight information about the proportion of correct answers may be lost due to rounding to the billionths place in results_combined.csv. To get practice question information, use '--requestions' ('-rq').")
				...
				print("No existing results_combined.csv found. All questions will be reprocessed.")

		questions_file_list = [file for file in file_list if undir.sub('\\4', file) not in q_already_processed]

	if ((os.path.isfile(Path(subj_quest_file_name)) or os.path.isfile(Path(summary_file_name))) and 
		not args.overwrite):
