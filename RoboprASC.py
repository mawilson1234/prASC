...
parser.add_argument('-rd', '--robodoc', default = False, action = "store_true",
	help = "Optional argument to process fix aligned ASCs to DA1s using Robodoc before SideEye analysis.")
parser.add_argument('-rr', '--rerobodoc', default = False, action = "store_true",
	help = "Optional argument to reprocess existing da1s from fix aligned ASCs using Robodoc.")
...
parser.add_argument('-v', '--verbose', default = False, action="store_true", 
	help = "Optional argument to print detailed information to the console.")
...
with Path(os.path.dirname(os.path.realpath(__file__))) as current_dir:
	...
	# Validate the parameters if we're using Robodoc and processing sentences
	if args.robodoc:

		if not 'robodoc_loc' in globals():
			robodoc_loc = current_dir / 'Robodoc.py'

		if not re.match('.*\.py$', str(robodoc_loc).lower()):
			robodoc_loc = Path(str(robodoc_loc) + '.py')

		while not os.path.isfile(robodoc_loc):
			robodoc_loc = Path(input("Error: no Robodoc.py file found. Please enter a valid location: "))
			if not robodoc_loc:
				robodoc_loc = current_dir / "Robodoc.py"
			if not re.match('.*\.py$', str(robodoc_loc).lower()):
				robodoc_loc = Path(str(robodoc_loc) + '.py')

		if not 'da1_output_dir' in globals():
			da1_output_dir = Path(asc_files_dir) / 'da1 files'

		if not os.path.exists(da1_output_dir):
			os.makedirs(da1_output_dir)

		# Check if there's a script file to pull parameters from
		if not 'script_loc' in globals():
			script_loc = "".join([f for f in os.listdir(current_dir) if '.script' in f])
		...
		# Get DC value from script file: it's easy; if "region = (...)" appears in it, it's 1, else 0
		# If we don't have a script file, make sure to sanitize this
		if not script_loc:
			if not 'DC' in globals():
				DC = input("Error: not specified whether there are display changes, and no script is provided. Enter 1 if any trial has a display change, 0 otherwise: ")
			
			# Loop until we get a valid setting for display changes
			while not isinstance(DC, int) or not(DC == 0 or DC == 1):
				try:
					if str(DC).lower() == "true" or str(DC).lower() == 't' or DC == True or int(DC) == 1:
						DC = 1
					elif str(DC).lower() == "false" or str(DC).lower() == 'f' or DC == False or int(DC) == 0:
						DC = 0
					DC = int(DC)
					if not(DC == 0 or DC == 1):
						raise Exception
			...
			script = open(script_loc, 'r').readlines()
			DC = 0
			for line in script:
				if re.match(r'^\s*region.*$', line):
					DC = 1
					break

		# If we have display changes, check the relevant parameters are correctly formatted
		if DC == 1:
			if not'min_change_time' in globals():
				min_change_time = -7
		...
		if not 'auto_exclude_DC' in globals():
			auto_exclude_DC = 0

		while not isinstance(auto_exclude_DC, int) or not(auto_exclude_DC == 0 or auto_exclude_DC == 1):
			try:
				if str(auto_exclude_DC).lower() == "true" or str(auto_exclude_DC).lower() == 't' or auto_exclude_DC == True or int(auto_exclude_DC) == 1:
					auto_exclude_DC = 1
				elif str(auto_exclude_DC).lower() == "false" or str(auto_exclude_DC).lower() == 'f' or auto_exclude_DC == False or int(auto_exclude_DC) == 0:
					auto_exclude_DC = 0
		...
		if not 'auto_exclude' in globals():
			auto_exclude = 0

		while not isinstance(auto_exclude, int) or not(auto_exclude == 0 or auto_exclude == 1):
			try:
				if str(auto_exclude).lower() == "true" or str(auto_exclude).lower() == "t" or auto_exclude == True or int(auto_exclude) == 1:
					auto_exclude = 1
				elif str(auto_exclude).lower() == "false" or str(auto_exclude).lower() == "f" or DC == False or int(auto_exclude) == 0:
					auto_exclude = 0
		...
		if auto_exclude == 1:
			if not 'exclude_threshold' in globals():
				exclude_threshold = 4

			while not isinstance(exclude_threshold, int) or not(exclude_threshold >= 0):
				try:
			...
			if not 'abs_exclude_threshold' in globals():
				abs_exclude_threshold = 9

			while not isinstance(abs_exclude_threshold, int) or not(abs_exclude_threshold >= 0):
				try:
		...
		if not 'blink_num_crit' in globals():
			blink_num_crit = 1000

		while not isinstance(blink_num_crit, int) or not(blink_num_crit >= 0):
			try:
		...
		if not 'blink_dur_crit' in globals():
			blink_dur_crit = 10000

		while not isinstance(blink_dur_crit, int) or not(blink_dur_crit >= 0):
			try:
		...
		if not 'blink_reg_exclude' in globals():
			blink_reg_exclude = "n"

		if blink_reg_exclude == "":
			blink_reg_exclude = "n"

		while not isinstance(blink_reg_exclude, str) or not (blink_reg_exclude == "y" or blink_reg_exclude == "n"):
			try:
				blink_reg_exclude = str(blink_reg_exclude).lower()
				if blink_reg_exclude == True or blink_reg_exclude == "true" or blink_reg_exclude == 't' or blink_reg_exclude == 1 or blink_reg_exclude == 'yes':
					blink_reg_exclude = "y"
				elif blink_reg_exclude == False or blink_reg_exclude == "false" or blink_reg_exclude == 'f' or blink_reg_exclude == 0 or blink_reg_exclude == 'no':
					blink_reg_exclude = "n"
		...
		if blink_reg_exclude == 'y':
			if not 'blink_region' in globals():
				blink_region = input("Error: trials with blink on the critical region are excluded, but blink_reg_exclude is not set. Please enter the number corresponding to your critical region: ")

			while not isinstance(blink_region, int) or not blink_region >= 0:
				try:
		...
		if not 'blink_gopast' in globals():
			if blink_reg_exclude == 'y':
				blink_gopast = input("Error: blink_reg_exclude is set, but blink_gopast is not. Enter 0 to exclude trials with blinks during first pass on blink_region, 1 to exclude trials with blinks during gopast: ")
			else:
				blink_gopast = 0

		while not isinstance(blink_gopast, int) or not (blink_gopast == 0 or blink_gopast == 1):
			try:
				if str(blink_gopast).lower() == "fp" or str(blink_gopast).lower() == 'firstpass' or str(blink_gopast).lower() == "first pass":
					blink_gopast = 0
				elif str(blink_gopast).lower() == 'gp' or str(blink_gopast).lower() == "gopast" or str(blink_gopast).lower() == "go past":
					blink_gopast = 0
		...
		while not isinstance(saccade_dur_crit, int) or not saccade_dur_crit >= 0:
			try:
			...
			if saccade_dur_crit < 1000:
				if not 'blink_region' in globals():
					blink_region = input("Error: trials with a long saccade on the critical region are excluded, but blink_region is not set. Please enter the number corresponding to your critical region: ")
		...
		while not isinstance(short_crit, int) or not short_crit >= 1:
			try:
		...
		# Function to get highest and lowest condition numbers from sentences_txt in case they are not manually set
		def get_conds(sentences_txt_loc):
			sentences_txt = open(sentences_txt_loc, 'r').readlines()

			lowest = 0
			highest = 0
			
			for line in sentences_txt:
				cond = int(line.split(' ')[1])

				# Set up condition in case the lowest condition number is not one. It can't be zero, so this way we know that the lowest condition is a condition in the file and compare others to the initial one
				if lowest == 0:
					lowest = cond
		...
		if not 'lowest_cond' in globals():
			lowest_cond = ""

		if not 'highest_cond' in globals():
			highest_cond = ""

		if not lowest_cond or not highest_cond:
			if not 'sentences_txt_loc' in globals():
					sentences_txt_loc = current_dir / "sentences.txt"
		...
		# If we don't have a start points matrix and we don't have a script where we could try to find it
		...
		# Calculate multiline stuff
		if len(re.findall('c', start_pts)) > 1:
			multi_line = 'y'
			first_line_y = int(re.sub(r'^rbind\s*\(\s*c\s*\(\s*[0-9]+\s*,\s*([0-9]+).*$', '\\1', start_pts))
			second_line_y = int(re.sub(r'^rbind\s*\(\s*c\s*\(\s*[0-9]+\s*,\s*[0-9]+\s*\)\s*,\s*c\s*\(\s*[0-9]+\s*,\s*([0-9]+).*$', '\\1', start_pts))
			line_sep_y = second_line_y - first_line_y
		else:
			multi_line = 'n'
...
if args.robodoc:
	...
	if not args.rerobodoc:
		file_list = [str(Path(fa_output_dir) / file) for file in os.listdir(Path(fa_output_dir)) if '.asc' in file and not re.sub(r'\.asc', '.da1', file) in os.listdir(Path(da1_output_dir))]
	else:
		file_list = [str(Path(fa_output_dir) / file) for file in os.listdir(Path(fa_output_dir)) if '.asc' in file]
	...
	if file_list:
		...
		fa_output_dir = da1_output_dir
		file_list = os.listdir(fa_output_dir)
		file_list = [str(Path(fa_output_dir) / f) for f in file_list if '.da1' in f]
	else:
...
if not args.nosentences:
	...
	if not args.robodoc:
		...
		file_list = os.listdir(da1_output_dir)
		file_list = [str(Path(da1_output_dir) / f) for f in file_list if '.da1' in f]
	...
	if not args.resentences:
		...
		undir = re.compile(r'(^[A-Z]:)?(\/|\\)?.*(\/|\\)((.*)\.(asc|da1)$)')
		...
		if not s_existing_results.empty and is_filename_included:
			...
			s_already_processed = [undir.sub('\\5', subj) for subj in s_already_processed]
	...
	if sentences_file_list:
		...
		sideeye.calculate_all_measures(sideeye_files, csv_loc, sideEyeConfig)
	else:
	...
	if not args.resentences and s_already_processed and not all(file in s_already_processed for file in [undir.sub('\\5', f) for f in file_list]) and os.path.isfile(csv_loc):
...
if not args.noquestions:
	...
	if not 'orig_fa_output_dir' in globals():
		file_list = os.listdir(fa_output_dir)
		file_list = [str(Path(fa_output_dir) / f) for f in file_list if '.asc' in f]
		questions_file_list = file_list
	else:
		questions_file_list = os.listdir(orig_fa_output_dir)
		questions_file_list = [str(Path(orig_fa_output_dir) / f) for f in questions_file_list if '.asc' in f]
	...
	if not args.requestions:
		...
		try:
			...
			q_already_processed = [undir.sub('\\5', subj) for subj in q_already_processed]
			...
			existing_subj_quest = existing_subj_quest[filename_col_name].isin([subj for subj in existing_subj_quest[filename_col_name].unique().tolist() if undir.sub('\\5', subj) in q_already_processed])
			existing_summary_file = existing_summary_file[filename_col_name].isin([subj for subj in existing_summary_file[filename_col_name].unique().tolist() if undir.sub('\\5', subj) in q_already_processed])
		except:
		...
		questions_file_list = [re.sub(r'\.da1$', '.asc', file) for file in questions_file_list if undir.sub('\\5', file) not in q_already_processed]
	...
	if not args.requestions and q_already_processed and not all(file in q_already_processed for file in [undir.sub('\\5', f) for f in file_list]) and os.path.isfile(subj_quest_file_name) and os.path.isfile(summary_file_name):
...
if not args.nocombine:
	...
	if os.path.isfile(csv_loc):
		...
		if os.path.isfile(subj_quest_file_name) and os.path.isfile(summary_file_name) and is_filename_included:
			results[filename_col_name] = results[filename_col_name].str.replace(r'(^[A-Z]:)?(\/|\\)?.*(\/|\\)((.*)\..*$)', '\\5')
			...
			questions[filename_col_name] = questions[filename_col_name].str.replace(r'(^[A-Z]:)?(\/|\\)?.*(\/|\\)((.*)\..*$)', '\\5')
