...
# Written by Adrian Staub, with mods by Jesse Harris, and Chuck Clifton (with further modifications by Michael Wilson)
...
#else:	
#	filename = input("What is the name of your Robodoc parameter file?")
# ----------------------------------
...
if blink_reg_exclude == "y":
	try:
		region_file = open(reg_file_loc, 'r')
	except:
...
os.makedirs(Path(da1_output_dir), exist_ok=True)
sumsub = open(Path(da1_output_dir) / 'sum.sub', 'w')
...
#for filename in file_list:
	#replace file extension with .da1
	#output_da1 = re.sub(r'\.[^.]+$', '.da1', filename)
...
# ------------------------
for filename in file_list:
	print("Processing " + re.sub(r'(.*?)([^\/\\]+$)', '\\2', filename) + " with Robodoc...")
...
# set up to keep record of actual position in sequence
	...
	#replace file extension with .da1
	output_da1 = re.sub(r'\.[^.]+$', '.da1', filename)
	output_da1 = Path(re.sub(r'(.*?)([^\/\\]+$)', '\\2', output_da1))

#open the new file
	os.makedirs(da1_output_dir, exist_ok=True) 
	output_da1 = open(Path(da1_output_dir) / output_da1, 'w')
	
	#do the same things to make a blink file
	output_bli = re.sub(r'\.[^.]+$', '.bli', filename)
	output_bli = Path(re.sub(r'(.*?)([^\/\\]+$)', '\\2', output_bli))
	os.makedirs(Path(da1_output_dir) / 'blink_files', exist_ok=True)	
	output_bli = open(Path(da1_output_dir) / 'blink_files' / output_bli, 'w')

# and if display change, a display change file
	if DC == 1:
		output_DC = re.sub(r'\.[^.]+$', '.dc', filename)
		output_DC = Path(re.sub(r'(.*?)([^\/\\]+$)', '\\2', output_DC))
		os.makedirs(Path(da1_output_dir) / 'DC_files', exist_ok=True)	
		output_DC = open(Path(da1_output_dir) / 'DC_files' / output_DC, 'w')
...
# now getting the trialid info
					...
					if int(cond_num) >= lowest_cond and int(cond_num) <= highest_cond and condition[0] != 'P': 
						#print(trialid, "after")
